...
let MailService = MailService_1 = class MailService {
    ...
    initTransporter() {
        const host = this.configService?.get('MAIL_HOST') || process.env.MAIL_HOST;
        const port = this.configService?.get('MAIL_PORT', 587) || Number(process.env.MAIL_PORT) || 587;
        const user = this.configService?.get('MAIL_USER') || process.env.MAIL_USER;
        const pass = this.configService?.get('MAIL_PASS') || process.env.MAIL_PASS;
        if (!host || !user || !pass) {
            this.logger.warn('Mail configuration missing (MAIL_HOST / MAIL_USER / MAIL_PASS). ' +
    ...
    async sendVerificationCode(toEmail, code, expiresAt) {
        const fromName = this.configService.get('MAIL_FROM_NAME', 'Memory Map');
        const fromEmail = this.configService.get('MAIL_USER') || 'noreply@memorymap.app';
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:40px 0;">
    <tr>
      <td align="center">
        ...
        const text = `Mã xác nhận Memory Map của bạn là: ${code}\n` +
        ...
        if (this.transporter) {
            try {
                ...
                this.logger.error(`Failed to send email to ${toEmail}: ${err.message}. Falling back to console log.`);
            }
        }
        this.logger.warn(`\n${'='.repeat(60)}`);
    ...
    async sendPasswordReset(toEmail, resetToken, resetUrl) {
        const fromName = this.configService.get('MAIL_FROM_NAME', 'Memory Map');
        const fromEmail = this.configService.get('MAIL_USER') || 'noreply@memorymap.app';
        const from = `"${fromName}" <${fromEmail}>`;
        const html = `
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:40px 0;">
    <tr>
      <td align="center">
        ...
        const text = `Yêu cầu đặt lại mật khẩu Memory Map\n\n` +
