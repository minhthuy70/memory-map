import { AlertCircle, Calendar, CheckCircle, RefreshCw, Settings, XCircle } from 'lucide-react';
'use client';
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white text-sm">Google Calendar</h3>
      ...
      <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-700 dark:text-slate-300">
            {isConnected ? 'Đã kết nối với Google Calendar' : 'Chưa kết nối với Google Calendar'}
          </span>
        </div>
      ...
      <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-lg">
        <p className="text-[10px] text-blue-700 dark:text-blue-400">
          <strong>Lưu ý:</strong> Đồng bộ sẽ tạo sự kiện Google Calendar từ kỷ niệm của bạn. 
          Kỷ niệm với ngày nhắc nhở sẽ được thêm vào lịch với thông báo.
        </p>
      </div>
    </div>
  );
