...
export default function IconPacks({ onCancel }: IconPacksProps) {
  ...
  const filteredPacks = selectedCategory === 'all' 
    ? iconPacks 
    : iconPacks.filter(pack => pack.category === selectedCategory);

  const categories = ['all', 'Basic', 'Travel', 'Food', 'Nature', 'Business'];

  const toggleInstall = (id: string) => {
    setIconPacks(iconPacks.map(pack => 
  ...
  const addIconPack = () => {
    ...
    setIconPacks([...iconPacks, newPack]);
  };

  const deletePack = (id: string) => {
    setIconPacks(iconPacks.filter(pack => pack.id !== id));
  };
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg text-center">
            ...
            <p className="text-lg font-bold text-slate-900 dark:text-white">{iconPacks.length}</p>
          </div>
          <div className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg text-center">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Installed</p>
            <p className="text-lg font-bold text-green-600 dark:text-green-400">{iconPacks.filter(p => p.isInstalled).length}</p>
          </div>
          <div className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg text-center">
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-700/30 border border-slate-200 dark:border-slate-600 rounded-lg">
          <h4 className="font-semibold text-slate-900 dark:text-white text-sm mb-3">Icon Packs ({filteredPacks.length})</h4>
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-700/30 border border-slate-200 dark:border-slate-600 rounded-lg">
          ...
          <div className="grid grid-cols-5 gap-2">
            <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 text-center">
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Basic</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{iconPacks.filter(p => p.category === 'Basic').length} packs</p>
            </div>
            <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 text-center">
              <p className="text-xs font-semibold text-blue-600 dark:text-blue-400 mb-1">Travel</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{iconPacks.filter(p => p.category === 'Travel').length} packs</p>
            </div>
            <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 text-center">
              <p className="text-xs font-semibold text-orange-600 dark:text-orange-400 mb-1">Food</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{iconPacks.filter(p => p.category === 'Food').length} packs</p>
            </div>
            <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 text-center">
              <p className="text-xs font-semibold text-green-600 dark:text-green-400 mb-1">Nature</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{iconPacks.filter(p => p.category === 'Nature').length} packs</p>
            </div>
            <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 text-center">
              <p className="text-xs font-semibold text-purple-600 dark:text-purple-400 mb-1">Business</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{iconPacks.filter(p => p.category === 'Business').length} packs</p>
            </div>
          </div>
        </div>
        ...
        {showDetails && (
          <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg">
            <h4 className="font-semibold text-slate-900 dark:text-white text-sm mb-3">Icon Pack Tips</h4>
            <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
              <li>• Icon packs customize memory categories</li>
              <li>• Use icons to organize memories visually</li>
              <li>• Free packs are available for all users</li>
              <li>• Premium packs offer unique designs</li>
            </ul>
          </div>
        )}
      </div>
