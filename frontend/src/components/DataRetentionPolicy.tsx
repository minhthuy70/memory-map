...
import { Clock, X, Settings, AlertTriangle } from 'lucide-react';

interface DataRetentionPolicyProps {
  onCancel?: () => void;
}

export default function DataRetentionPolicy({ onCancel }: DataRetentionPolicyProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-teal-50 dark:bg-teal-950/30 border border-teal-200 dark:border-teal-900 rounded-lg">
        <p className="text-xs text-teal-700 dark:text-teal-400">
          <strong>Đang phát triển:</strong> Chính sách lưu trữ dữ liệu cho phép xóa dữ liệu cũ theo lịch.
        </p>
      </div>
    </div>
  );
