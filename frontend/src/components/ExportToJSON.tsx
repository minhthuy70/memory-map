...
interface ExportToJSONProps {
  onCancel?: () => void;
  onExport?: (options: JSONOptions) => void;
}

interface JSONOptions {
  format: 'pretty' | 'compact' | 'minified';
...
export default function ExportToJSON({ onCancel, onExport }: ExportToJSONProps) {
  const [isExporting, setIsExporting] = useState(false);
  const [showSettings, setShowSettings] = useState(false);
  const [showPreview, setShowPreview] = useState(false);
  const [exportProgress, setExportProgress] = useState(0);
  const [exportStatus, setExportStatus] = useState<'idle' | 'generating' | 'serializing' | 'done' | 'error'>('idle');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Export memories as structured JSON data
            </p>
          </div>
        </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                >
                  ...
                  <option value="compact">Compact (no extra spaces)</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setOptions({ ...options, dateFormat: e.target.value as any })}
                  className="text-xs bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded px-2 py-1 w-full"
                >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setOptions({ ...options, version: parseInt(e.target.value) })}
                className="w-full"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => setOptions({ ...options, includeRelations: !options.includeRelations })}
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    options.includeRelations ? 'bg-slate-500' : 'bg-slate-300 dark:bg-slate-600'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                      options.includeRelations ? 'translate-x-5' : ''
                    }`}
                  />
                </button>
              </div>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    options.includeComments ? 'bg-slate-500' : 'bg-slate-300 dark:bg-slate-600'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                      options.includeComments ? 'translate-x-5' : ''
                    }`}
                  />
                </button>
              </div>
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => setOptions({ ...options, nullValues: !options.nullValues })}
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    options.nullValues ? 'bg-slate-500' : 'bg-slate-300 dark:bg-slate-600'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                      options.nullValues ? 'translate-x-5' : ''
                    }`}
                  />
                </button>
              </div>
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => setOptions({ ...options, customFields: !options.customFields })}
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    options.customFields ? 'bg-slate-500' : 'bg-slate-300 dark:bg-slate-600'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                      options.customFields ? 'translate-x-5' : ''
                    }`}
                  />
                </button>
              </div>
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => setOptions({ ...options, encryption: !options.encryption })}
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    options.encryption ? 'bg-slate-500' : 'bg-slate-300 dark:bg-slate-600'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                      options.encryption ? 'translate-x-5' : ''
                    }`}
                  />
                </button>
              </div>
      ...
      <div className="space-y-4">
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-700/30 border border-slate-200 dark:border-slate-600 rounded-lg">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-600 aspect-[3/4] flex items-center justify-center">
              <div className="text-center">
                <Database className="h-16 w-16 text-slate-300 dark:text-slate-600 mx-auto mb-2" />
        ...
        {exportStatus !== 'idle' && (
          <div className="p-4 bg-slate-50 dark:bg-slate-700/30 border border-slate-200 dark:border-slate-600 rounded-lg">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
              ...
              {exportStatus === 'serializing' && <Code className="h-3 w-3" />}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-lg font-medium hover:from-yellow-600 hover:to-yellow-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            {isExporting ? (
              <>
