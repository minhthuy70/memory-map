...
import { Trophy, X, Settings, AlertTriangle } from 'lucide-react';

interface MemoryChallengesProps {
  onCancel?: () => void;
}

export default function MemoryChallenges({ onCancel }: MemoryChallengesProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-lg">
        <p className="text-xs text-amber-700 dark:text-amber-400">
          <strong>Đang phát triển:</strong> Thử thách kỷ niệm tạo các thử thách để khuyến khích người dùng tạo kỷ niệm mới.
        </p>
      </div>
    </div>
  );
