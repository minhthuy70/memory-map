...
import { Folder, X, Settings, AlertTriangle } from 'lucide-react';

interface CategoryComparisonProps {
  onCancel?: () => void;
}

export default function CategoryComparison({ onCancel }: CategoryComparisonProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900 rounded-lg">
        <p className="text-xs text-orange-700 dark:text-orange-400">
          <strong>Đang phát triển:</strong> So sánh danh mục cho phép so sánh số lượng kỷ niệm giữa các danh mục khác nhau.
        </p>
      </div>
    </div>
  );
