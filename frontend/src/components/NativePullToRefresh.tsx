...
import { RefreshCw, X, Settings, AlertTriangle } from 'lucide-react';

interface NativePullToRefreshProps {
  onCancel?: () => void;
}

export default function NativePullToRefresh({ onCancel }: NativePullToRefreshProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 rounded-lg">
        <p className="text-xs text-green-700 dark:text-green-400">
          <strong>Đang phát triển:</strong> Pull-to-refresh native sử dụng native gesture để refresh dữ liệu.
        </p>
      </div>
    </div>
  );
