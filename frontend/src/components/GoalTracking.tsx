...
import { Target, X, Settings, AlertTriangle } from 'lucide-react';

interface GoalTrackingProps {
  onCancel?: () => void;
}

export default function GoalTracking({ onCancel }: GoalTrackingProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 rounded-lg">
        <p className="text-xs text-green-700 dark:text-green-400">
          <strong>Đang phát triển:</strong> Theo dõi mục tiêu cho phép đặt và theo dõi mục tiêu tạo kỷ niệm.
        </p>
      </div>
    </div>
  );
