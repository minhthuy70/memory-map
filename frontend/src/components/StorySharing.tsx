...
import { Story, X, Settings, AlertTriangle } from 'lucide-react';

interface StorySharingProps {
  onCancel?: () => void;
}

export default function StorySharing({ onCancel }: StorySharingProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-pink-50 dark:bg-pink-950/30 border border-pink-200 dark:border-pink-900 rounded-lg">
        <p className="text-xs text-pink-700 dark:text-pink-400">
          <strong>Đang phát triển:</strong> Chia sẻ story cho phép tạo và chia sẻ story từ kỷ niệm lên các nền tảng.
        </p>
      </div>
    </div>
  );
