...
import { Flame, X, Settings, AlertTriangle } from 'lucide-react';

interface MemoryHeatmapsProps {
  onCancel?: () => void;
}

export default function MemoryHeatmaps({ onCancel }: MemoryHeatmapsProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900 rounded-lg">
        <p className="text-xs text-orange-700 dark:text-orange-400">
          <strong>Đang phát triển:</strong> Bản đồ nhiệt kỷ niệm hiển thị phân phối kỷ niệm theo thời gian và địa điểm.
        </p>
      </div>
    </div>
  );
