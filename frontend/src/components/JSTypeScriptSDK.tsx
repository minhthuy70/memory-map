...
export default function JSTypeScriptSDK({ onCancel }: JSTypeScriptSDKProps) {
  const [showDetails, setShowDetails] = useState(false);
  const [isSDKEnabled, setIsSDKEnabled] = useState(true);

  const [sdkVersions, setSDKVersions] = useState<SDKVersion[]>([
    { id: '1', version: '2.5.0', type: 'stable', publishedAt: '2024-01-17', downloadCount: 15420, isLatest: true },
    { id: '2', version: '2.4.8', type: 'stable', publishedAt: '2024-01-10', downloadCount: 12850, isLatest: false },
    { id: '3', version: '2.5.1-beta', type: 'beta', publishedAt: '2024-01-18', downloadCount: 2340, isLatest: false },
  ]);
  ...
  const [sdkInstalls, setSDKInstalls] = useState<SDKInstall[]>([
    { id: '1', method: 'npm', command: 'npm install @memorymap/sdk', description: 'Install via npm' },
    { id: '2', method: 'yarn', command: 'yarn add @memorymap/sdk', description: 'Install via yarn' },
    { id: '3', method: 'pnpm', command: 'pnpm add @memorymap/sdk', description: 'Install via pnpm' },
  ]);
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <div className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg text-center">
            ...
            <p className="text-lg font-bold text-yellow-600 dark:text-yellow-400">{sdkVersions.find(v => v.isLatest)?.version || 'N/A'}</p>
          </div>
          <div className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg text-center">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Downloads</p>
            <p className="text-lg font-bold text-orange-600 dark:text-orange-400">{sdkVersions.reduce((acc, v) => acc + v.downloadCount, 0).toLocaleString()}</p>
          </div>
          <div className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg text-center">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Features</p>
            <p className="text-lg font-bold text-amber-600 dark:text-amber-400">{sdkFeatures.filter(f => f.isImplemented).length}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            type="button"
            className="px-3 py-1.5 rounded-lg text-xs bg-yellow-600 hover:bg-yellow-700 text-white border-0 flex items-center gap-1"
          >
        ...
        {showDetails && (
          <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg">
            <h4 className="font-semibold text-slate-900 dark:text-white text-sm mb-3">JavaScript/TypeScript SDK Tips</h4>
            <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
              <li>• Install via npm, yarn, or pnpm</li>
