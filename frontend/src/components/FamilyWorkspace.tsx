...
import { Home, X, Settings, AlertTriangle } from 'lucide-react';

interface FamilyWorkspaceProps {
  onCancel?: () => void;
}

export default function FamilyWorkspace({ onCancel }: FamilyWorkspaceProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900 rounded-lg">
        <p className="text-xs text-orange-700 dark:text-orange-400">
          <strong>Đang phát triển:</strong> Không gian làm việc gia đình cho phép gia đình tạo không gian chung để chia sẻ kỷ niệm.
        </p>
      </div>
    </div>
  );
