...
import { Baby, X, Settings, AlertTriangle } from 'lucide-react';

interface ChildAccountsProps {
  onCancel?: () => void;
}

export default function ChildAccounts({ onCancel }: ChildAccountsProps) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div className="p-4 bg-pink-50 dark:bg-pink-950/30 border border-pink-200 dark:border-pink-900 rounded-lg">
        <p className="text-xs text-pink-700 dark:text-pink-400">
          <strong>Đang phát triển:</strong> Tài khoản trẻ em cho phép tạo tài khoản an toàn cho trẻ em với kiểm soát phụ huynh.
        </p>
      </div>
    </div>
  );
