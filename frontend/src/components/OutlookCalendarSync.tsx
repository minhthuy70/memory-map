...
export default function OutlookCalendarSync({
  ...
  const handleExportICS = async () => {
    if (onExportICS) {
      ...
      a.href = url;
      a.download = `memory-map-outlook-calendar-${new Date().toISOString().split('T')[0]}.ics`;
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white text-sm">Outlook Calendar</h3>
      ...
      <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-700 dark:text-slate-300">
            {isConnected ? 'Đã kết nối với Outlook Calendar' : 'Chưa kết nối với Outlook Calendar'}
          </span>
        </div>
      ...
      <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-lg">
        <p className="text-[10px] text-blue-700 dark:text-blue-400">
          <strong>Lưu ý:</strong> Outlook Calendar hỗ trợ đồng bộ qua Microsoft 365. Bạn cũng có thể xuất file ICS 
          để nhập thủ công vào Outlook hoặc Outlook Web.
        </p>
      </div>
    </div>
  );
