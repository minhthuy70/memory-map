          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                {isEmailVerified ? (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800">
      ...
      {showVerifyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 max-w-md w-full relative">
            ...
            {debugCode && (
              <div className="mb-4 p-2.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl text-xs text-amber-700 dark:text-amber-300 flex items-center justify-between">
                <span>Mã xác nhận thử nghiệm: <strong className="font-mono font-bold tracking-wider">{debugCode}</strong></span>
                <button
                  type="button"
                  onClick={() => {
                    setVerificationCode(debugCode.split(''));
                  }}
                  className="text-[11px] underline font-medium hover:text-amber-900 dark:hover:text-amber-100 cursor-pointer ml-2"
                >
                  Tự động điền
                </button>
              </div>
